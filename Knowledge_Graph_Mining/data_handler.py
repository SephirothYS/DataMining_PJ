...
def get_type_relation_head(N):                                                          #对rehandle的数据前N条生成types + h_关系的record,该数据可以使用FPgrowth算法进行处理                                       
...
def transform_rules(rules,alias_dict,savename = None,save_flag=False,show=False):       #对上述两个函数生成的文件进行具体名称转换，可保存结果，也可仅输出结果
    if save_flag == True:
...
def connect_rules(headpath,tailpath,savepath):                                      #读取handle_head_result，handle_tail_result函数产生文件,并对关系项相同项进行连接                           
