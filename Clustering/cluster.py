...
def plot_cluster(result,newData,numClass):                          #聚类数据可视化
        ...
        #          'g^'] * 5 
        colors = ['#F0F8FF','#00FFFF','#000000','#FFE4C4','#A52A2A','#FF7F50','#D2691E','#DC143C','#8B0000','#FF8C00'] * 11
...
def output_result(s):
    fp = open("cluster_99class_Direct_TSNE2_TF-IDF.jsonl","w",encoding="utf-8")
...
if __name__ == "__main__":
    ...
    # data = pickle.load(fp)
    fp = open("./Sent_vector_data/Direct_TSNE2_TF-IDF.pkl","rb")               #选择源数据，该数据已经进行了降维
    ...
    def save_score(score1_list,score2_list,score3_list):                            #保存score
        fp = open("./score_result/cluster_score_SC_TF-IDF.pkl","wb")
