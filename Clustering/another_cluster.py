...
def plot_cluster(result,newData,numClass):                  #聚类数据可视化
        ...
        for labi in result:
            ...
            index += 1
        colors = ['#9400D3','#00FFFF','#000000','#FFE4C4','#A52A2A','#FF7F50','#D2691E','#DC143C','#8B0000','#FF8C00'] * 11
...
if __name__ == "__main__":
    ...
    fp = open("./Sent_vector_data/Direct_TSNE2_TF-IDF.pkl","rb")
    ...
    for k in range(N):
        ...
        # cluster_instance = fcm(Tnewdata,center)                                            #选取聚类算法
        # cluster_instance = kmeans(Tnewdata,center)
        # cluster_instance = clique(Tnewdata,10,3)
        # cluster_instance = birch(Tnewdata,70,diameter=2.0)
        cluster_instance = cure(Tnewdata,num_class)
        # cluster_instance = rock(Tnewdata,1.0,70)
        ...
        if score > max:                                                             #多轮迭代时保存最佳轮廓系数对应数据
            filename = "./Cluster_Result/cluster_" + str(num_class + k) + "class_cure_TSNE_TF-IDF.jsonl"
